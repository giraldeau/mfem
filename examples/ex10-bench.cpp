...
void run(Mesh *mesh, int order, int dim, int ode_solver_type, real_t visc,
   real_t mu, real_t K, bool visualization, real_t dt, real_t t_final,
   int vis_steps) {
...
int main(int argc, char *argv[])
{
   ...
   std::vector<std::pair<std::string, int>> solver_types = {
      {"Forward Euler", 1},
      {"RK2(0.5", 2},
      {"RK3 SSP", 3},
      {"RK4", 4},
      ...
      {"SDIRK(2)", 22},
      {"SDIRK33", 23},
      {"Implicit Midpoint", 32},
      {"SDIRK23", 33},
      {"SDIRK34", 34},
      {"Generalized Alpha", 40},
      {"AM1", 51},
      {"AM2", 52},
      {"AM3", 53},
      {"AM4", 54}
   ...
   std::vector<double> elapsed_time(solver_types.size(), 0.0);

   for (int i = 0; i < solver_types.size(); i++) {
      auto solver_name = solver_types[i].first;
      auto solver_id = solver_types[i].second;
      std::cout << "Solver " << solver_name << " " << solver_id << std::endl;
      auto t1 = std::chrono::high_resolution_clock::now();
      run(mesh, order, dim, solver_id, visc, mu, K, visualization, dt, t_final, vis_steps);
